...
var RESERVED_PROPRIETY = ["extendClass"];
...
function writeProperty_js(obj){
    data = "";
    for (k in obj) {
        var val = obj[k];
        if (k == "extendClass"){
            data += writeProperty_js(objDictionnary[val]);
            continue;
        }
        data += "\n       this." + k + " = ";
...
function generateFileClass_gd() {
    ...
    // REMOVE PREVIUS GENERATED FILES
    var files = fs.readdirSync(generatedPath_gd);
    for (var i = 0; i < files.length; i++)
        fs.unlinkSync(generatedPath_gd + "/" + files[i]);
...
function add_gd_getData(obj, path) {
    ...
    if (obj !== null && Array.isArray(obj)) {
        ...
        for (k in obj){
            if (k == "extendClass"){
                data += add_gd_getData(objDictionnary[obj[k]], path);
                continue;
            }
...
function add_gd_setData(obj, path) {
    ...
    if (obj !== null && Array.isArray(obj)) {
        ...
        for (k in obj){
            if (k == "extendClass"){
                data += add_gd_setData(objDictionnary[obj[k]], path);
                continue;
            }
